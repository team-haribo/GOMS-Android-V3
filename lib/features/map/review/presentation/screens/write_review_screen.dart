...
class _WriteReviewScreenState extends ConsumerState<WriteReviewScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(writeReviewViewModelProvider.notifier).reset();
    });
    _writeReviewSubscription = ref.listenManual<WriteReviewState>(
      ...
      (previous, next) async {
        ...
        if (next.status == WriteReviewStatus.success) {
          ...
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (!mounted) return;
            final notifier = ref.read(writeReviewViewModelProvider.notifier);
            notifier.clearError();
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(writeReviewViewModelProvider);
    final notifier = ref.read(writeReviewViewModelProvider.notifier);
