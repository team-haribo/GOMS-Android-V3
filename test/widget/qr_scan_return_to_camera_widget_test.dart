...
/// #122 회귀 테스트.
///
/// 실패 결과 화면의 "카메라로 돌아가기"가 `context.go(QrRoutePath.qr)`였을 때는
...
void main() {
  testWidgets('QR 실패 화면에서 카메라로 돌아가면 스캔 화면 아래 스택이 유지된다', (tester) async {
    ...
    router.push(QrRoutePath.qr);
    await tester.pumpAndSettle();
    router.push(QrRoutePath.qrResultLocation('failure'));
    await tester.pumpAndSettle();
  ...
  testWidgets('스캔 화면 없이 실패 화면에 바로 진입하면 스캔 화면으로 이동한다', (tester) async {
    final router = GoRouter(
      initialLocation: QrRoutePath.qrResultLocation('failure'),
      routes: [
        GoRoute(
          path: QrRoutePath.qr,
          builder: (context, state) => const Scaffold(body: Text('scan-screen')),
